...
kernel void bitonic_sort_per_tile_kernel(
    ...
    int sort_count = min(count_raw, MAX_TILE_ELEMS);
    int raw_end = tile_offsets[tg_id];
    int raw_start = raw_end - sort_count;
    int total_intersections = tile_offsets[num_tiles - 1];
    int start = raw_start;
    int end = raw_end;

    // If the packed working set is full, proportionally thin every tile. This
    // preserves a coherent whole frame instead of emptying later image tiles.
    if (total_intersections > (int)output_capacity) {
        float packed_scale = (float)output_capacity / (float)total_intersections;
        start = min((int)((float)raw_start * packed_scale), (int)output_capacity);
        end = tg_id == num_tiles - 1
            ? (int)output_capacity
            : min((int)((float)raw_end * packed_scale), (int)output_capacity);
    }
    int write_count = max(end - start, 0);

    // Keep every output range inside the packed buffers. Without this guard,
    // dense late-training scenes can overwrite unrelated GPU memory.
    if (tid == 0) {
        write_packed_int2(tile_bins, tg_id, int2(start, end));
        if (tg_id == 0 && total_intersections > (int)output_capacity) {
            atomic_store_explicit(overflow_flag, 1u, memory_order_relaxed);
        }
    }

    if (sort_count == 0 || write_count == 0) return;

    // Round up to next power of 2
    int n = 1;
    while (n < sort_count) n <<= 1;
    ...
    for (int i = (int)tid; i < n; i += SORT_TG_SIZE) {
        data[i] = (i < sort_count) ? prealloc_bins[bin_base + i] : 0xFFFFFFFFFFFFFFFFULL;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int i = (int)tid; i < write_count; i += SORT_TG_SIZE) {
