...
struct FusedTensorCache {
    ...
    int chunk_K_max = 0;
    int chunk_img_height = 0, chunk_img_width = 0;
...
static void forward_pipeline(
    ...
    // --- Overflow check: detect per-tile or packed-output intersection overflow ---
    // Only warn once to avoid noisy output in very dense scenes.
    ...
    if (!overflow_warned && g_tcache.overflow_flag.defined() && g_tcache.fwd_num_points > 0
        ...
        if (flag_val > 0) {
            fprintf(stderr, "WARNING: tile intersection capacity exceeded. "
                    "Some gaussians were safely dropped from the render.\n");
            overflow_warned = true;
        }
    }
    int64_t capacity = std::min(
        (int64_t)num_points * g_tcache.capacity_multiplier,
        (int64_t)num_tiles * 2048
    );
    uint32_t channels = 3;
